...
edge_files = [
    'CongregationManagement.tsx',
    'KomisiManagement.tsx',
    'AttendanceManagement.tsx',
    'KomselManagement.tsx'
]
...
with open('src/lib/edgeKvClient.ts', 'w', encoding='utf-8') as f:
    f.write(edge_mock.strip())


def process_file(filepath):
    filename = os.path.basename(filepath)
    if filename == 'supabaseAdmin.ts': return
    with open(filepath, 'r', encoding='utf-8') as f:
        content = f.read()

    if 'supabaseAdmin' in content:
        if filename in edge_files:
            # Replace import
            content = re.sub(
                r"import\s*\{\s*supabaseAdmin\s*\}\s*from\s*['\"](?:\.\./)+lib/supabaseAdmin['\"];?",
                "import { EdgeKV as supabaseAdmin } from '../../../lib/edgeKvClient';",
                content
            )
        else:
            # Replace import with normal supabase client
            if "import { supabase }" in content:
                # Just remove supabaseAdmin import and replace usages
                content = re.sub(r"import\s*\{\s*supabaseAdmin\s*\}\s*from\s*['\"](?:\.\./)+lib/supabaseAdmin['\"];?\n?", "", content)
                content = content.replace("supabaseAdmin", "supabase")
            else:
                # Add import and replace usages
                content = re.sub(
                    r"import\s*\{\s*supabaseAdmin\s*\}\s*from\s*['\"](?:\.\./)+lib/supabaseAdmin['\"];?",
